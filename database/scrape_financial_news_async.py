#!/usr/bin/env python3
"""
Script ASYNCHRONE pour scraper les données financières des entreprises
Version optimisée avec workers parallèles (6 workers par défaut)
"""
...
MAX_CONCURRENT_COMPANIES = 6  # 6 entreprises en parallèle
...
async def scrape_company_financial_news(company_name: str) -> Dict[str, Any]:
    """
    Scrape financial news pour une entreprise (ASYNC)
    """
...
Return ONLY the JSON object with the exact structure specified."""
        ...
        print(f"  ✅ {company_name}: {len(financial_items)} items")
...
def save_results_periodically(financial_data: Dict[str, Any], output_file: str):
...
async def process_all_companies():
    """
    Traite toutes les entreprises en parallèle avec workers limités
    """
    ...
    print(f"\n{'='*80}")
    print(f"🚀 SCRAPING ASYNCHRONE - FINANCIAL NEWS")
    print(f"{'='*80}")
    print(f"📊 {len(companies)} entreprises à traiter")
    print(f"👥 {MAX_CONCURRENT_COMPANIES} workers en parallèle")
    print(f"{'='*80}\n")
    ...
    for coro in asyncio.as_completed(tasks):
        try:
            ...
            if completed % 5 == 0 or completed == total:
                save_results_periodically(financial_data, output_file)
    ...
    total_items = sum(len(company_data.get("financial_items", [])) for company_data in financial_data.values())
    successful = sum(1 for company_data in financial_data.values() 
                     if company_data.get("scrape_metadata", {}).get("success"))
    
    print(f"\n📈 Statistiques:")
    print(f"   - Entreprises traitées: {len(financial_data)}")
    print(f"   - Succès: {successful}")
    print(f"   - Total items financiers: {total_items}")
    print(f"   - Moyenne par entreprise: {total_items/len(financial_data):.1f}")


async def test_single_company(company_name: str):
    """Test sur une seule entreprise"""
    
    print(f"🧪 Test ASYNC sur {company_name}...")
    
    financial = await scrape_company_financial_news(company_name)
    
    # Sauvegarde du test
    with open("financial_news_test.json", 'w', encoding='utf-8') as f:
        json.dump({company_name: financial}, f, indent=2, ensure_ascii=False)
    
    print(f"\n✅ Test terminé!")
    print(f"📁 Résultat sauvegardé dans financial_news_test.json")
    print(f"\n📊 Résumé:")
    print(f"   - Items financiers trouvés: {len(financial.get('financial_items', []))}")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='Scrape financial news (ASYNC)')
    parser.add_argument('mode', nargs='?', default='full', help='Mode: test or full (default: full)')
    parser.add_argument('--company', type=str, help='Company name to scrape (for test mode)')
    parser.add_argument('--days', type=int, help='Number of days to look back (for API compatibility)')
    
    args = parser.parse_args()
    
    if args.mode == "test" or args.company:
        # Mode test sur une seule entreprise
        company = args.company or "MillerKnoll"
